...
window.BuildingMap=(function(){
 ...
 // Availability is field-specific: an upper use does not fill the ground use,
 // and a lower-bound floor count is not a completed total. No source writes.
 function gaps(key,r){
  if(key==='surveyuse')return [!r.ground&&!r.autoGroundUses?.length?'G?':null,!r.use&&!r.autoUpperUse?'U?':null].filter(Boolean);
  if(key==='surveystoreys')return r.storeys?[]:[r.autoStoreysMinimum?'total?':'F?'];
  if(key==='surveyera')return r.era?[]:['Y?'];
  if(key==='surveygreen')return r.green?[]:['V?'];
  return [];
 }
 function coverage(key,m=model()){
  const placed=new Set(m.features.flatMap(f=>f.recordIds)),rows=[...m.records.values()].filter(e=>e.shape.kind!=='site').map(e=>{
   const missing=gaps(key,e.record),available=hasField(key,e.record,e);
   return {id:e.record.id,name:e.record.note||e.record.autoEvidence?.title||e.record.id,mapped:placed.has(e.record.id),photos:proofs(e,key).length,missing,status:missing.length?(available?'partial':'missing'):'available'};
  });return {key,rows,unrecordedOutlines:m.features.filter(f=>!f.recordIds.length).length,openSpace:m.openSpace.length};
 }
 ...
 function render(key){
  ...
  m.features.forEach(f=>{
   ...
   if(isField){const es=f.recordIds.map(id=>m.records.get(id)).filter(e=>e&&(key!=='pictures'||hasField(key,e.record,e)));if(es.length===1){entry=es[0];const r=entry.record;unknown=!hasField(key,r,entry);reviewed=!!r.autoAttributeReview&&AutoAttributes.applied(key,r);if(unknown)unknownIds.add(r.id);else mappedIds.add(r.id);if(reviewed)autoIds.add(r.id);const cat=key==='pictures'||unknown?null:category(key,r);partial=!!cat?.partial;if(partial)minimumIds.add(r.id);color=unknown?'#b07b34':cat?.color||'#5d7566';stroke=unknown?color:key==='surveyuse'&&!r.use&&!r.autoUpperUse?'#819294':cat?.upperColor||color;label=unknown?gaps(key,r).join(' '):key==='pictures'?(r.storeys?r.storeys+'F':r.autoEvidence?.label||'•'):key==='surveyuse'?gaps(key,r).join(' '):cat?.short;opacity=unknown?.07:partial||key==='pictures'||key==='surveyuse'&&!r.ground&&!r.autoGroundUses?0:.45;}else if(es.length>1)conflicts.push(...es);}
   ...
   if(color){const missing=entry?gaps(key,entry.record):[],p=L.polygon(f.rings,{color:stroke||color,weight:unknown?1:entry?1.8:.8,fillColor:color,fillOpacity:opacity,dashArray:unknown||reviewed?'4 3':null,renderer,bmRole:'building-analysis',bmId:f.id,bmRecordId:entry?.record.id||null,bmMissingFields:missing,bmAttributeStatus:unknown?'unknown':partial?'minimum':missing.length?'partial':reviewed?'auto-reading':'recorded'}).addTo(group);painted.push(f.id);if(entry){wire(p,entry,key);if(label){const marker=ArchitecturalMap.label(f.at,label+(!missing.length&&!partial&&!unknown&&(f.approx||reviewed)?'≈':''),{height:4.5,box:true,color:missing.length&&!partial?'#966322':'#344f54',className:'bm-tag',bmRecordId:entry.record.id,bmMissingFields:missing}).addTo(group);wire(marker,entry,key);}}else if(key==='noise')p.bindPopup('Building proximity to the mapped highway centreline. This is not measured sound or air pollution.');}
  });
  ...
  if(['siteplot','edges'].includes(key)){
   ...
   if(key==='siteplot'){
    const labels=[[[33.89630,35.54101],[33.89620,35.54088],'SOUTH · HIGHWAY','#a75d46'],[[33.89651,35.54130],[33.89648,35.54154],'EAST · ARRIVAL','#2f807b'],[[33.89674,35.54117],[33.89690,35.54115],'NORTH · NEIGHBOUR','#8d719b'],[[33.89655,35.54097],[33.89655,35.54064],'WEST · NEIGHBOUR','#8d719b']];
    labels.forEach(([at,to,title,color])=>{L.polyline([at,to],{color,weight:.7,interactive:false,renderer,bmRole:'edge-label-leader'}).addTo(group);ArchitecturalMap.label(to,title,{height:2.3,color,interactive:false,className:'bm-edge-label'}).addTo(group);});
   }
  }
  ...
  if(key==='siteplot'){rows.splice(0,rows.length,['#f2dea8','Our unbuilt project plot'],['#a75d46','South · highway-facing edge'],['#2f807b','East · Street 80 arrival edge'],['#8d719b','North / west · neighbour edges']);}
  ...
  const msg=document.createElement('p');msg.className='bm-small';msg.textContent=isField?(key==='pictures'?'Original survey records and exact linked photographs.':'Dashed colour = screenshot reading / VERIFY. Amber ? = this field is missing, even if a photo exists. Plain grey outline = no linked building record. '+(key==='surveystoreys'?'≥ = minimum visible levels, NOT total floors; these stay unfilled. ≈ = estimate / approximate outline.':key==='surveyuse'?'G? = ground use missing; U? = upper / building use missing.':key==='surveyera'?'Y? = construction date unknown. A photograph is not proof of a year.':'V? = greenery not assessed, not proof there is none.')):key==='grain'?'Relative sizes of display outlines, not legal parcel areas. Approximate rectangles influence this comparison.':key==='noise'?'Geometric proximity only—not a sound, exposure or safety rating.':'Corrected building base · approximate display geometry. Roads and our site remain separate.';legend.appendChild(msg);
  ...
  if(isField&&key!=='pictures'){
   const c=coverage(key,m),placed=c.rows.filter(r=>r.mapped),counts=Object.fromEntries(['available','partial','missing'].map(s=>[s,placed.filter(r=>r.status===s).length])),d=document.createElement('details');d.className='bm-coverage';
   d.innerHTML='<summary>Evidence gaps · '+counts.missing+' missing · '+counts.partial+' partial</summary><p>'+counts.available+' mapped records have this field; '+counts.partial+' are partial and '+counts.missing+' are missing. Recorded values and photo interpretations are not independently verified. '+c.unrecordedOutlines+' grey context outlines have no linked building record; '+c.rows.filter(r=>!r.mapped).length+' building records are not placed. Open-space records are excluded.</p>';
   [['Mapped: missing or partial',c.rows.filter(r=>r.mapped&&r.missing.length)],['Not on a building outline',c.rows.filter(r=>!r.mapped)]].forEach(([title,items])=>{
    if(!items.length)return;const heading=document.createElement('h4');heading.textContent=title;d.appendChild(heading);
    items.forEach(r=>{const line=document.createElement('p');line.className='bm-small';line.textContent=r.name+' — '+(r.missing.join(' / ')||'field recorded')+' · '+(r.photos?'linked photo':'no linked photo')+'. ';const b=document.createElement('button');b.textContent='Review evidence';b.onclick=()=>{api.go('picturesauto');window.picturesAuto.open(r.id,true);};line.appendChild(b);d.appendChild(line);});
   });panel.appendChild(d);
  }
  if(key==='pictures'){
   const pending=[...m.records.values()].filter(e=>hasField(key,e.record,e)&&!mappedIds.has(e.record.id)),d=document.createElement('details');d.className='bm-coverage';d.innerHTML='<summary>'+mappedIds.size+' mapped original records · '+pending.length+' not on a building outline</summary><p>Unplaced records and open-space observations remain separate. No nearby building is assigned automatically.</p>';
   pending.forEach(e=>{const b=document.createElement('button');b.textContent=e.record.note||e.record.id;b.onclick=()=>{L.popup({maxWidth:360}).setLatLng(api.map.getCenter()).setContent(popupNode(e,key)).openOn(api.map);hydrate(e,key);};d.appendChild(b);});panel.appendChild(d);
  }
  if(key!=='pictures'){
 ...
 return {configure(a){api=a;},model,base,shade,fit,render,contains,center,area,keys,gaps,coverage,recordAt(p){const m=model(),hits=m.features.filter(f=>contains(p,f.rings));if(hits.length!==1||hits[0].recordIds.length!==1)return null;return api.records().find(r=>r.id===hits[0].recordIds[0])||null;},refresh(){if(current)render(current);}};
})();
