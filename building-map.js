...
window.BuildingMap=(function(){
 'use strict';
 let api=null,cache=null,signature='',current=null,reference=false,solarSeason='summer',solarHour=12,windFrom=225,legacyRenderer=null;
 const keys=['history','siteplot','figureground','grain','edges','surveyuse','surveystoreys','surveyera','surveygreen','surveychange','pictures','streets','access','noise','sun','wind'];
 ...
 function note(group,at,text,color='#344f54'){L.marker(at,{interactive:false,icon:L.divIcon({className:'bm-label',html:'<span style="color:'+color+'">'+esc(text)+'</span>',iconSize:[150,28],iconAnchor:[75,14]})}).addTo(group);}
 function arrow(group,start,end,color){L.polyline([start,end],{color,weight:2.5,dashArray:'7 5',interactive:false}).addTo(group);const a=api.map.latLngToLayerPoint(start),b=api.map.latLngToLayerPoint(end),t=Math.atan2(b.y-a.y,b.x-a.x);L.polyline([api.map.layerPointToLatLng([b.x-12*Math.cos(t-.5),b.y-12*Math.sin(t-.5)]),end,api.map.layerPointToLatLng([b.x-12*Math.cos(t+.5),b.y-12*Math.sin(t+.5)])],{color,weight:2.5,interactive:false}).addTo(group);}
 function fit(key){if(!api||!keys.includes(key))return false;api.map.invalidateSize();const close=['siteplot','edges'].includes(key),bounds=close?L.latLngBounds(B().site).pad(.65):B().bounds;api.map.fitBounds(bounds,{paddingTopLeft:[22,35],paddingBottomRight:[22,45],maxZoom:close?19:17,animate:false});return true;}
 ...
 function hasField(key,r,e){if(key==='pictures')return api.records().some(o=>o.id===r.id)&&(!api.missingOnly()||!api.shots(r.id).length);return key==='surveyuse'?!!(r.use||r.ground):key==='surveystoreys'?!!r.storeys:key==='surveyera'?!!r.era&&(e.shots.length||api.shots(r.id).length):key==='surveygreen'?!!r.green:['y','n'].includes(r.added)||['y','n'].includes(r.ready);}
 ...
 function popup(entry,key){
  ...
  const field=key==='pictures'?api.card(r,{[r.id]:shots}):api.fieldRows(key,r);
 ...
 function render(key){
  ...
  const group=api.groups[key],renderer=legacyRenderer||(legacyRenderer=L.canvas({padding:.4})),m=base(group,renderer),isField=key.startsWith('survey')||key==='pictures',painted=[],mappedIds=new Set(),conflicts=[];
  ...
  m.features.forEach(f=>{
   let color=null,opacity=.44,label=null,entry=null,stroke=null;
   if(isField){const es=f.recordIds.map(id=>m.records.get(id)).filter(e=>hasField(key,e.record,e));es.forEach(e=>mappedIds.add(e.record.id));if(es.length===1){entry=es[0];const r=entry.record,cat=key==='pictures'?null:api.category(key,r);color=cat?.color||'#5d7566';stroke=key==='surveyuse'&&!r.use?'#819294':cat?.upperColor||color;label=key==='pictures'?(r.storeys?r.storeys+'F':r.autoEvidence?.label||'•'):key==='surveyuse'?null:cat?.short;opacity=key==='pictures'||key==='surveyuse'&&!r.ground?0:.45;}else if(es.length>1)conflicts.push(...es);}
   ...
   if(color){const p=L.polygon(f.rings,{color:stroke||color,weight:entry?1.8:.8,fillColor:color,fillOpacity:opacity,renderer,bmRole:'building-analysis',bmId:f.id,bmRecordId:entry?.record.id||null}).addTo(group);painted.push(f.id);if(entry){wire(p,entry,key);if(label){const marker=L.marker(f.at,{icon:L.divIcon({className:'bm-tag',html:'<span>'+esc(label)+(f.approx?'≈':'')+'</span>',iconSize:[64,20],iconAnchor:[32,10]}),bmRecordId:entry.record.id}).addTo(group);wire(marker,entry,key);}}else if(key==='noise')p.bindPopup('Building proximity to the mapped highway centreline. This is not measured sound or air pollution.');}
  });
  ...
  if(key==='sun'){const s=window.protectionSolar(solarSeason,solarHour),end=point(ctr,s.azimuth,190);arrow(group,ctr,end,'#bd873a');note(group,end,'Toward the sun · '+solarSeason,'#8c612c');}
  if(key==='wind'&&windFrom!=='calm'){const start=point(ctr,Number(windFrom),210);arrow(group,start,ctr,'#387f89');note(group,start,'Wind FROM · scenario','#387f89');}
  ...
  if(reference)(B().alignedFaces||[]).forEach(f=>{if(f)L.polygon(f.g||f,{color:'#927a9c',weight:.7,fill:false,dashArray:'3 6',opacity:.6,interactive:false,renderer,bmRole:'source-reference'}).addTo(group);});
  const legend=document.getElementById('map-legend');legend.innerHTML='<b class="ml-title">'+esc(api.title(key))+'</b>';
  const rows=isField?(key==='pictures'?[['#5d7566','Original records · click one building']]:api.legend(key).map(r=>[r.literal||api.css(r.color),r.label.replace(/parcel/gi,'building outline')])):key==='figureground'?[['#40585e','Building footprints · not land parcels']]:key==='grain'?[['#dacabb','Smaller building outlines'],['#ad9277','Middle-sized building outlines'],['#755e48','Larger building outlines']]:key==='noise'?[['#a65345','Closest to highway centreline'],['#c88568','Intermediate proximity'],['#dfb5a0','Further within the study band']]:key==='sun'?[['#bd873a','Idealised sun direction · not a shadow study']]:key==='wind'?[['#387f89','Direction scenario · not measured wind']]:[['#a35f45','Main road corridor'],['#39817f','Local road'],['#a63f69','Mapped footway / stairs']];
  let rowHost=legend;if(key==='surveyuse'){const p=document.createElement('p');p.className='bm-small';p.textContent='Fill = recorded ground-floor use. Outline = use above / building use. Click a building for its exact record.';legend.appendChild(p);const d=document.createElement('details');d.innerHTML='<summary>Use colours</summary>';legend.appendChild(d);rowHost=d;}
  ...
  const msg=document.createElement('p');msg.className='bm-small';msg.textContent=isField?'Colour stays inside the assigned building outline. Blank = not recorded or not reliably placed.':key==='grain'?'Relative sizes of display outlines, not legal parcel areas. Approximate rectangles influence this comparison.':key==='noise'?'Geometric proximity only—not a sound, exposure or safety rating.':'Corrected building base · approximate display geometry. Roads and our site remain separate.';legend.appendChild(msg);
  const ref=document.createElement('label');ref.className='bm-reference';ref.innerHTML='<input type="checkbox" '+(reference?'checked':'')+'> Old parcel reference (optional)';ref.querySelector('input').onchange=e=>{reference=e.target.checked;render(key);};legend.appendChild(ref);
  if(key==='sun'||key==='wind'){const controls=document.createElement('div');controls.className='bm-controls';controls.innerHTML=key==='sun'?'<label>Season<select id="bm-season"><option value="summer">Summer</option><option value="equinox">Equinox</option><option value="winter">Winter</option></select></label><label>Solar hour<select id="bm-hour"><option>9</option><option>12</option><option>15</option></select></label>':'<label>Wind comparison<select id="bm-wind"><option value="225">From south-west</option><option value="257">From west-south-west</option><option value="337">From north-north-west</option><option value="calm">Calm / unknown</option></select></label>';legend.appendChild(controls);if(key==='sun'){document.getElementById('bm-season').value=solarSeason;document.getElementById('bm-hour').value=solarHour;document.getElementById('bm-season').onchange=e=>{solarSeason=e.target.value;render(key);};document.getElementById('bm-hour').onchange=e=>{solarHour=+e.target.value;render(key);};}else{document.getElementById('bm-wind').value=windFrom;document.getElementById('bm-wind').onchange=e=>{windFrom=e.target.value;render(key);};}}
  ...
  if(isField){const pending=[...m.records.values()].filter(e=>hasField(key,e.record,e)&&!mappedIds.has(e.record.id));const d=document.createElement('details');d.className='bm-coverage';d.innerHTML='<summary>'+mappedIds.size+' records mapped · '+pending.length+' not on a building outline</summary><p>Includes unplaced records and observations about open space. No nearby building is assigned automatically. Their records and photos are preserved.</p>';pending.forEach(e=>{const b=document.createElement('button');b.textContent=e.record.note||e.record.id;b.onclick=()=>{L.popup({maxWidth:360}).setLatLng(api.map.getCenter()).setContent(popupNode(e,key)).openOn(api.map);hydrate(e,key);};d.appendChild(b);});panel.appendChild(d);}
  window.BUILDING_MAP_AUDIT={key,features:m.features.length,painted,mappedIds:[...mappedIds],conflicts:conflicts.map(e=>e.record.id),unplaced:m.unplaced.map(e=>e.record.id),source:'Pictures Auto corrected display geometry'};
 }
