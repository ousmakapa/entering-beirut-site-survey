...
window.BuildingMap=(function(){
 ...
 function fit(key){if(!api||!keys.includes(key))return false;api.map.invalidateSize();const close=['siteplot','edges'].includes(key),climate=['sun','wind'].includes(key),ctr=center([B().site]),bounds=close?L.latLngBounds(B().site).pad(.65):climate?L.latLngBounds([point(ctr,0,290),point(ctr,90,290),point(ctr,180,290),point(ctr,270,290)]):B().bounds;api.map.fitBounds(bounds,{paddingTopLeft:[22,35],paddingBottomRight:[22,45],maxZoom:close?19:17,animate:false});return true;}
