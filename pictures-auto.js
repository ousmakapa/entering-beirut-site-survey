...
window.createPicturesAuto = function(api){
  ...
  function card(r){
    ...
    return (a?'<section class="pa-evidence"><h4>Auto evidence · '+esc(A.collectedOn)+'</h4><p>'+esc(a.summary)+'</p>'+(a.location?'<p><b>F · frontage reference only.</b> '+esc(a.location.basis)+'</p>':'')+'<dl>'+a.observations.map(function(o){return '<dt>'+esc(o.field)+'</dt><dd>'+esc(o.value)+'<br><small>'+esc(o.certainty)+' · '+esc(o.basis)+'</small></dd>';}).join('')+'</dl><p><b>Still to verify:</b> '+esc(a.unresolved)+'</p></section>':'<p class="pa-banner">Copied record — no new Auto evidence collected yet.</p>')+
      '<section class="pa-original"><h4>Original survey · unchanged copy</h4><dl>'+field('Record',r.id)+field('Old plot',r.parcel)+field('Storeys',r.storeys)+field('Use',r.use)+field('Ground floor',r.ground)+field('Apparent construction',r.era?r.era+' · visual estimate / VERIFY':'Not estimated')+field('Construction basis',r.eraBasis)+field('Greenery',r.green)+field('Green coverage',r.gcover)+field('Green access',r.gaccess)+field('Visible additions',r.added==='y'?'Yes':r.added==='n'?'No':'Not assessed')+field('Columns for another floor',r.ready==='y'?'Yes':r.ready==='n'?'No':'Not assessed')+'</dl></section>';
  }
  ...
  var state=view.stats;view.stats=function(){return Object.assign(state(),{autoEvidence:A.additions.length,newScreenshots:A.additions.reduce(function(n,a){return n+a.screenshots.length;},0),baselineRecords:records.length});};
  return view;
};
